...
import { readSessionTiming } from './session-timing';
...
const configSchema = z.object({
  ...
  auth: z.object({
    ...
    // Validated in ./session-timing, which owns the env parsing and the
    // updateAge < expiresIn rule for every consumer.
    sessionExpiresIn: z.number().int().positive(),
    sessionUpdateAge: z.number().int().positive(),
