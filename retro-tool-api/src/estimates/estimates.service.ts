...
export class EstimatesService {
  ...
  /**
   * Whether the user belongs to the session's team. WebSocket join/vote handlers
   * call this before joining a room or mutating a session — handshake auth only
   * proves identity, not membership, so without this any authenticated socket
   * could join any session by ID and cast votes (SECURITY-ASSESSMENT F2). Mirrors
   * the membership gate in the REST `getSession` path. Unlike `canManageSession`,
   * this returns true for plain members, not just leads/admins.
   */
  ...
  async getSession(userId: string, sessionId: string): Promise<SessionDetail> {
    ...
    const roundViews = allRounds.map((round) => {
      ...
      return {
        ...
        votes: roundVotes,
        stats: computeRoundStats(roundVotes),
      };
    });
    ...
    const result: SessionDetail = {
      ...
      currentStory: currentRound
        ? roundStoryLabel(currentRound.storyName, currentRound.ticketNumber)
  ...
  async updateStory(
    ...
    await this.database
      .update(estimatesSchema.storyEstimateSession)
      .set({
        currentStory: roundStoryLabel(data.storyName, data.ticketNumber),
        updatedAt: new Date(),
      })
      .where(eq(estimatesSchema.storyEstimateSession.id, sessionId));
