...
@Injectable()
export class StandupsProjectionSyncService {
    ...
    await Promise.all(
      members.map(async ({ userId }) => {
        try {
          ...
          this.logger.warn(
            `Convex standup board snapshot sync failed for standupId=${standupId} date=${date} userId=${userId}: ${message}`,
          );
        }
      }),
    );
  ...
  private async runMutation(path: string, args: object): Promise<void> {
    ...
    try {
      ...
      this.logger.warn(
        `Convex standup projection mutation ${path} failed: ${message}`,
      );
    }
  }
}
