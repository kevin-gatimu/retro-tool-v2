...
| ----------- | ------------------------------------------------------ | ----------- | -------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------- |
| Production  | `environment=prod`                                   | `main`    | `retro_tool` (legacy — predates this Bicep; see [`infra/provisioning.md`](../infra/provisioning.md)) | **Yes** — automatic on push to `main` via `release-production.yml` |
...
(production, called automatically by [`release-production.yml`](../../.github/workflows/release-production.yml) on push to `main`).
...
| --------------------------------------------------------------------------------------- | ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`release-staging.yml`](../../.github/workflows/release-staging.yml)                   | orchestrator (staging)       | On push to `staging` (path-filtered) or dispatch: runs `convex` → `api` → `ui` in order against staging                                                                                                                                                                     |
| [`release-production.yml`](../../.github/workflows/release-production.yml)             | orchestrator (production)    | On push to `main` (path-filtered) or dispatch: runs `convex-production` → `api` → `ui` in order against production                                                                                                                                                          |
...
| [`deploy-convex-production.yml`](../../.github/workflows/deploy-convex-production.yml) | `retrotool-prod-convex`    | Same stop-first sequence as staging, against`infra/convex-production.bicep` / the `retro_tool` resource group. Called automatically by [`release-production.yml`](../../.github/workflows/release-production.yml) on push to `main`; `workflow_dispatch` available for out-of-band Convex-only deploys |
