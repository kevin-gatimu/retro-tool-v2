...
| [configuration.ts](../../retro-tool-api/src/config/configuration.ts)                                                                             | Env parsing: auth secret/url, JWT issuer/audience, rpId/rpName, Microsoft, Convex. Delegates session-timing parsing to `session-timing.ts`.                          |
| [config/session-timing.ts](../../retro-tool-api/src/config/session-timing.ts)                                                                    | Centralised Zod-validated parser for `BETTER_AUTH_SESSION_EXPIRES_IN` / `BETTER_AUTH_SESSION_UPDATE_AGE`. Enforces `updateAge < expiresIn`. Called by `configuration.ts` and `auth.ts`. |
...
| `auth.sessionExpiresIn`               | `BETTER_AUTH_SESSION_EXPIRES_IN` (default `1800`)    | Rolling session lifetime in seconds; validated by [`session-timing.ts`](../../retro-tool-api/src/config/session-timing.ts) |
| `auth.sessionUpdateAge`               | `BETTER_AUTH_SESSION_UPDATE_AGE` (default `300`)     | Minimum interval between server-side expiry renewals in seconds; must be < `sessionExpiresIn` or the API throws at startup |
